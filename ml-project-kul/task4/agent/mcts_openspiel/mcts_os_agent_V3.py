...
class MCTSAgent():
    ...
    def _apply_tree_policy(self, root, state):
        ...
        while (not working_state.is_terminal() and current_node.explore_count > 0) or ( working_state.is_chance_node() and self.dont_return_chance_node):
            #state_id = working_state.serialize()
            #with self.transposition_lock: 
            #    if state_id in self.transposition_table:
            #        return self.transposition_table[state_id]  
            ...
            #with self.transposition_lock:
            #    self.transposition_table[state_id] = (visit_path, working_state)
...
def get_agent_for_tournament(player_id):
    ...
    epsilon = 0.25  # weight for noise
    alpha = 0.03  # From alphazero paper

    agent = MCTSAgent(game, exploration_coefficient, num_simulations, evaluator, dirichlet_noise=(epsilon, alpha))

    logging.info(f"MCTSAgent OS V3 created for tournament with player ID: {player_id}")
    return agent
