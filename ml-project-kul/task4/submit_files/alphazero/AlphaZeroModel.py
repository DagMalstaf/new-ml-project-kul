...
class AlphaZeroModel(Model):
    ...
    def select_best_action(self, state):
        ...
        prediction = self.predict(input, verbose=False)
        policy = prediction['policy'][0]
        legal_actions = state.legal_actions()
        legal_actions_set = set(legal_actions)
        
        while True:
            best_action = np.argmax(policy)
            if best_action in legal_actions_set:
                return best_action
            else:
                policy[best_action] = -float('inf') 
...
class NNetWrapper:
    ...
    def select_best_action(self, state):
        return self.model.select_best_action(state)
