...
def main():
  ...
  max_game_iterations:int = 200000
  ...
  def temperature_reduction_function(count: int, tau: float) -> float:
    min_tau = 0.2
    max_iter: int = 20000
  ...
  if "Q_learning" in learning_name:
    ...
    if "lenient" in learning_name:
      #graph.render_lenient_plot(game, Agents_list, alpha= alpha, tau = tau, kappa = kappa, normalise= normalise_vector_plot)
        ...
        #graph.render_combined_2_actions_graph_FAQ(game, all_traces, initial_Q, alpha, gamma, tau, learning_name, kappa = kappa, normalise=normalise_vector_plot)
